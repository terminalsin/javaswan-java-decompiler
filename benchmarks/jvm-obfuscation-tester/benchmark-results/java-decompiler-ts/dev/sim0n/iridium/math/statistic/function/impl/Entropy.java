...
public class Entropy extends AbstractStatisticFunction {
  ...
  public double evaluate(double[] data) {
    ...
    double entropy = data;
    int var6 = entropy.length;
    for (int var7 = 0; var7 < var6; var7++) {
      double value = entropy[var7];
      valueCounts.put(
          value, (Integer) valueCounts.computeIfAbsent(value, arg -> 0).intValue() + 1);
    }
    entropy = valueCounts.values().stream()
        .mapToDouble(arg -> (double) freq.intValue() / n)
        .map(arg -> arg * (Math.log(arg) / LN_2))
        .sum();
    entropy = -entropy;
    return entropy;
  }
