...
public class Blowfish {
  ...
  private String encStr(String sPlainText, long lNewCBCIV) {
    ...
    BlowfishCBC blowfishCBC = this.m_bfish;
    synchronized (blowfishCBC) {
      this.m_bfish.setCBCIV(lNewCBCIV);
      this.m_bfish.encrypt(buf);
    }
    byte[] newCBCIV = new byte[8];
    Blowfish.longToByteArray(lNewCBCIV, newCBCIV, 0);
    return Blowfish.bytesToBinHex(newCBCIV, 0, 8) + Blowfish.bytesToBinHex(buf, 0, buf.length);
  }
  ...
  public String decryptString(String sCipherText) {
    ...
    return Blowfish.byteArrayToUNCString(buf, 0, nNumOfBytes);
  }
  ...
  private static class BlowfishECB {
    ...
    public BlowfishECB(byte[] bfkey) {
      ...
      while (nI < 18) {
        for (int nJ = 0; nJ < 4; ++nJ) {
          ...
          if (++nKeyPos != nLen) continue;
          nKeyPos = 0;
        }
        int n = nI++;
        this.m_pbox[n] = this.m_pbox[n] ^ nBuild;
      }
      long lZero = 0L;
      for (nI = 0; nI < 18; nI += 2) {
    ...
    protected long encryptBlock(long lPlainBlock) {
      ...
      nLo ^= (sbox1[(nHi ^= pbox[0]) >>> 24] + sbox2[nHi >>> 16 & 0xFF] ^ sbox3[nHi >>> 8 & 0xFF])
              + sbox4[nHi & 0xFF]
          ^ pbox[1];
    ...
    protected long decryptBlock(long lCipherBlock) {
      ...
      nLo ^= (this.m_sbox1[(nHi ^= this.m_pbox[17]) >>> 24] + this.m_sbox2[nHi >>> 16 & 0xFF]
