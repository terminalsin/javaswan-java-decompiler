...
public class Blowfish {
  ...
  public Blowfish(String password) {
    ...
    this.m_bfish = new Blowfish.BlowfishCBC(digest.digest(), 0L);
    digest.reset();
  }
      ...
      if (nNumOfBytes < 8) {
        ...
        if (nLen == 0) {
          ...
          if (nNumOfBytes < nLen) {
            ...
            int nPadByte = buf[buf.length - 1] & 255;
            if (nPadByte > 8 || nPadByte < 0) {
              nPadByte = 0;
            }

            nNumOfBytes -= nPadByte;
            return nNumOfBytes < 0 ? "" : byteArrayToUNCString(buf, 0, nNumOfBytes);
          }
        }
      }
    }
  ...
  private static long byteArrayToLong(byte[] buffer, int nStartIndex) {
    return (long) buffer[nStartIndex] << 56
        | (buffer[nStartIndex + 1] & 255L) << 48
        | (buffer[nStartIndex + 2] & 255L) << 40
        | (buffer[nStartIndex + 3] & 255L) << 32
        | (buffer[nStartIndex + 4] & 255L) << 24
        | (buffer[nStartIndex + 5] & 255L) << 16
        | (buffer[nStartIndex + 6] & 255L) << 8
        | buffer[nStartIndex + 7] & 255L;
  }

  private static void longToByteArray(long lValue, byte[] buffer, int nStartIndex) {
    buffer[nStartIndex] = (byte) (lValue >>> 56);
    buffer[nStartIndex + 1] = (byte) (lValue >>> 48 & 255L);
    buffer[nStartIndex + 2] = (byte) (lValue >>> 40 & 255L);
    buffer[nStartIndex + 3] = (byte) (lValue >>> 32 & 255L);
    buffer[nStartIndex + 4] = (byte) (lValue >>> 24 & 255L);
    buffer[nStartIndex + 5] = (byte) (lValue >>> 16 & 255L);
    buffer[nStartIndex + 6] = (byte) (lValue >>> 8 & 255L);
    buffer[nStartIndex + 7] = (byte) lValue;
  }

  private static long intArrayToLong(int[] buffer, int nStartIndex) {
    return (long) buffer[nStartIndex] << 32 | buffer[nStartIndex + 1] & 4294967295L;
  }
  ...
  private static long makeLong(int nLo, int nHi) {
    return (long) nHi << 32 | nLo & 4294967295L;
  }
  ...
  private static int binHexToBytes(
      String sBinHex, byte[] data, int nSrcPos, int nDstPos, int nNumOfBytes) {
  ...
  private static String byteArrayToUNCString(byte[] data, int nStartPos, int nNumOfBytes) {
    ...
    for (int nSBufPos = 0; nNumOfBytes > 0; nNumOfBytes -= 2) {
  ...
  private static class BlowfishCBC extends Blowfish.BlowfishECB {
    long m_lCBCIV;
    ...
    private long decryptBlockCBC(long lCipherblock) {
      long var5 = super.decryptBlock(lCipherblock);
      long var6 = var5 ^ this.m_lCBCIV;
      this.m_lCBCIV = lCipherblock;
      return var6;
    }
  ...
  private static class BlowfishECB {
    ...
    int[] m_pbox = new int[18];
    int[] m_sbox1;
    int[] m_sbox2;
    int[] m_sbox3;
    int[] m_sbox4;
    static final int[] pbox_init = new int[] {
      608135816,
      -2052912941,
      320440878,
      57701188,
    ...
    static final int[] sbox_init_1 = new int[] {
      -785314906,
      -1730169428,
      805139163,
      -803545161,
    ...
    static final int[] sbox_init_2 = new int[] {
      1266315497,
      -1246549692,
      -613086930,
      -1004984797,
    ...
    static final int[] sbox_init_3 = new int[] {
      -381855128,
      -1803468553,
      -162781668,
      -1805047500,
    ...
    static final int[] sbox_init_4 = new int[] {
      976866871,
      -738527793,
      -1413318857,
      1522871579,
    ...
    public BlowfishECB(byte[] bfkey) {
      ...
      for (int var9 = 0; var9 < 256; var9++) {
        this.m_sbox1[var9] = sbox_init_1[var9];
        this.m_sbox2[var9] = sbox_init_2[var9];
        this.m_sbox3[var9] = sbox_init_3[var9];
        this.m_sbox4[var9] = sbox_init_4[var9];
      }
      ...
      if (nLen != 0) {
        ...
        for (int var10 = 0; var10 < 18; var10++) {
          ...
          this.m_pbox[var10] = this.m_pbox[var10] ^ nBuild;
        }

        long lZero = 0L;

        for (int var11 = 0; var11 < 18; var11 += 2) {
          lZero = this.encryptBlock(lZero);
          this.m_pbox[var11] = (int) (lZero >>> 32);
          this.m_pbox[var11 + 1] = (int) (lZero & 4294967295L);
        }

        for (int var12 = 0; var12 < 256; var12 += 2) {
          lZero = this.encryptBlock(lZero);
          this.m_sbox1[var12] = (int) (lZero >>> 32);
          this.m_sbox1[var12 + 1] = (int) (lZero & 4294967295L);
        }

        for (int var13 = 0; var13 < 256; var13 += 2) {
          lZero = this.encryptBlock(lZero);
          this.m_sbox2[var13] = (int) (lZero >>> 32);
          this.m_sbox2[var13 + 1] = (int) (lZero & 4294967295L);
        }

        for (int var14 = 0; var14 < 256; var14 += 2) {
          lZero = this.encryptBlock(lZero);
          this.m_sbox3[var14] = (int) (lZero >>> 32);
          this.m_sbox3[var14 + 1] = (int) (lZero & 4294967295L);
        }

        for (int var15 = 0; var15 < 256; var15 += 2) {
          lZero = this.encryptBlock(lZero);
          this.m_sbox4[var15] = (int) (lZero >>> 32);
          this.m_sbox4[var15 + 1] = (int) (lZero & 4294967295L);
        }
      }
    }
    ...
    public void cleanUp() {
      ...
      for (int var2 = 0; var2 < 256; var2++) {
        this.m_sbox1[var2] = this.m_sbox2[var2] = this.m_sbox3[var2] = this.m_sbox4[var2] = 0;
      }
    }
    ...
    public static boolean selfTest() {
      ...
      if (tv_t1[0] == tv_c1[0] && tv_t1[1] == tv_c1[1]) {
        ...
        if (tv_t1[0] == tv_p1[0] && tv_t1[1] == tv_p1[1]) {
          Blowfish.BlowfishECB testbf2 = new Blowfish.BlowfishECB(testKey2);
